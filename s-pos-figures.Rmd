...
(ggplot(map2,aes(fill=n))+geom_sf(size=0.3)+
    scale_fill_gradient(trans="log1p", breaks = c(0,3,10,30,100,300,1000,3000,10000), low="#FFFFFF",high="#FF8000",name="Pillar 2 positives")+
    labs(subtitle = paste0(startLLdate," to ",lastLLdate))
...
aocDf %>% rename(name = ltlaName) %>% select(area,code,name,population) %>% mutate(population = sprintf("%1.0f",population)) %>% group_by(area) %>% standardPrintOutput::saveTable(filename=here::here("output/areasOfConcernTable"))
...
lapply(names(clusters2), function(name) {
  doEpiCurve(combinedSpositives, aocDf %>% filter(area %in% clusters2[[name]])) %>% standardPrintOutput::saveFigure(paste0(here::here("output/AOCMultiplot-"),name,"-",Sys.Date()),maxWidth = 8,maxHeight = 6)
})
...
countsBySgene2 = 
  ...
  combinedPlot %>% standardPrintOutput::saveHalfPageFigure(filename=paste0(here::here("output/caseCountsAndSequenceCountsBySgene-"),Sys.Date()))
```
...
tmp2 %>% readr::write_csv(paste0(here::here("output/raw-demographics-line-list-by-area-of-concern-"),Sys.Date(),".csv"))


tmp3 = tmp2 %>% mutate(
  date = date + sample.int(6,n(),replace = TRUE) - 3,
  age = age + sample.int(6,n(),replace = TRUE) - 3
)  %>%
filter(date>="2020-10-01" & date < max(tmp2$date) & !is.na(area)) %>% 
select(date,sGene,area,LTLA_code,LTLA_name,age)
  
tmp3 %>% readr::write_csv(paste0(here::here("output/synthetic-demographics-line-list-by-area-of-concern-"),Sys.Date(),".csv"))

```  


#tmp3 %>% readr::write_csv(paste0(here::here("output/age-distributions-all-areas-"),Sys.Date(),".csv"))
#aocDf %>% readr::write_csv(paste0(here::here("output/area-definitions-"),Sys.Date(),".csv"))
