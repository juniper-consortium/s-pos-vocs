...
```{r}
    ...
    breaks = mapVoc %>% 
      #filter(category==cat & count>0) %>% 
      filter(count>0) %>% 
      pull(count) %>% quantile(c(0.5,0.75,0.9,0.95,0.975))
...
mapP1 = lapply(categories$category, function(cat) {    
    tmp = mapVoc %>% 
      filter(category==cat) %>% 
      mutate(count2=cut(count,breaks = c(-Inf,0,breaks,Inf),labels = plotlabels,include.lowest = FALSE))
    ...
    if(cat == "S-gene positive") {
      p = p +
        ggrepel::geom_label_repel(
          data = bufferedAocLabels,# %>% mutate(category="S-gene positive" %>% ordered(c("B.1.617.2","S-gene positive","non B.1.617.2 VOC"))),# %>% filter(pos=="left"), 
